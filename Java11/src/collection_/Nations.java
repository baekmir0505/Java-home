...
class ArrayList4 {
	...
	// 변경하고자 하는 기존 값, 변경 후의 값 (중국 -> 캐나다)
	public void setNation(String prev, String after) {
		// set(방번호, 변경값) : 이전 값의 방번호를 알아야함 그래서 인덱스오프 사용
		int number = nations.indexOf(prev);     // 이전 값의 번호를 알아냄
		nations.set(number, after);          // number 방번호에 있는 값을 after로 바뀜
	}
	
	// 항목을 삭제하는 함수(indexOf, remove)
	public void deleteNation(String str) {
		// remove(방번호) : 방번호로 삭제를 하니깐 입력값의 방번호를 알아냄
		int number = nations.indexOf(str);    // 입력한 문자열이 갖고 있는 방번호를 찾음
