...
class MyClass5{
	...
	void 메서드1() {    
		...
		// 오로지 코드 재사용
		int num = 1;
		System.out.println("나느 메서드"+num+"에 저장된 코드다");
	}
	
	// 메서드2 : 반환값X, 입력값O (코드 재활용, 일부반 변수인 경우)
	...
	// 메서드3 : 반환값O, 입력값X (코드 재사용, 사용 후에 마지막으로 전달해줄 값이 있음)
	String 메서드3() {
		// 메서드의 자료형은 return(반환)값의 자료형을 적어준다
		String result = "반갑습니다.";
		System.out.println("메서드3을 사용!");
		
		return result;         // 코드마지막에 return 옆에 있는 값을 전달
	}
